...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		try{
			dialogue = CoolUtil.coolStringFile(Assets.getText(Paths.txt(SONG.song.toLowerCase() + '/dialogue')));
		} catch(err){
			dialogue = null;
			trace('NO DIALOGUE '+err);
		}

		switch(SONG.song.toLowerCase())
		{
			case 'repeat' | 'jacky' | 'staircase' | 'doubles' | 'last-save' | 'stay':
            {
                    defaultCamZoom = 1;
                    curStage = 'labDoor';

                    var backdrop = Paths.getSparrowAtlas('lab/DoorBG');

                    BG = new FlxSprite(0, -500);
                    BG.frames = backdrop;
                    BG.animation.addByPrefix('idle', 'basicBG');
                    BG.animation.addByPrefix('light', 'lightUpBG', 24, false);
                    BG.animation.addByPrefix('door', 'openBG', 24, false);
            ...
            {
                defaultCamZoom = 0.9;
                curStage = 'examinationRoom';

                var backdrop = Paths.getSparrowAtlas('lab/BGIGuess');

                BG = new FlxSprite(-320, -100);
                BG.frames = backdrop;
                BG.animation.addByPrefix('idle', 'Rocky_BG', 24, true);
                BG.animation.play('idle');
                BG.antialiasing = true;
                add(BG);
            }
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " - KE " + MainMenuState.kadeEngineVer + " - " + (FlxG.save.data.etternaMode ? "E.Mode" : "FNF"), 16);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(SONG.song.toLowerCase() == 'curiosity'){
						if(misses < 10)
							FlxG.switchState(new WinImage(Paths.image('end/good'), new MainMenuState()));
						else if(misses < 50)
							FlxG.switchState(new WinImage(Paths.image('end/bad'), new MainMenuState()));
						else
							FlxG.switchState(new WinImage(Paths.image('end/bad'), new PlayState(), true));
					} else {
	...
	*/
	...
	override function beatHit()
	{
		...
		if (SONG.song.toLowerCase() == 'repeat')
		{
			switch (curBeat)
			{
				case 84:
				...
				case 86:
					BG.animation.play('door');
			}
		}
		if (SONG.song.toLowerCase() == 'jacky')
		{
			switch (curBeat)
			{
				case 92:
		...
		if (SONG.song.toLowerCase() == 'staircase')
		{
			switch (curBeat)
			{
				case 108:
		...
		if (SONG.song.toLowerCase() == 'doubles')
		{
			switch (curBeat)
			{
				case 112:
		...
		if (SONG.song.toLowerCase() == 'last-save')
		{
			switch (curBeat)
			{
				case 184:
